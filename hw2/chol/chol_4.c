            ...
            for (k = 0; k < j; ) {
                if (k < max(j - BLKSIZE, 0)) { // OPT 2
                    tmp -= A[IDX(i, k, n)] * A[IDX(j, k, n)] +
                           ...
                           A[IDX(i, k+8, n)] * A[IDX(j, k+8, n)] +
                           A[IDX(i, k+9, n)] * A[IDX(j, k+9, n)] +
                           A[IDX(i, k+10, n)] * A[IDX(j, k+10, n)] +
                           A[IDX(i, k+11, n)] * A[IDX(j, k+11, n)] +
                           A[IDX(i, k+12, n)] * A[IDX(j, k+12, n)] +
                           A[IDX(i, k+13, n)] * A[IDX(j, k+13, n)] +
                           A[IDX(i, k+14, n)] * A[IDX(j, k+14, n)] +
                           A[IDX(i, k+15, n)] * A[IDX(j, k+15, n)];
                    k += BLKSIZE;
                } else {
                    tmp -= A[IDX(i, k, n)] * A[IDX(j, k, n)];
                    k++;
